...
async function generateRekapKehadiranForMonth(month, year, globalConfig) {
    ...
    // 2. Clear data area merges (>= 15)
    //    We also clear rows 3-14 merges to re-extend them
    const allMergeKeys = Object.keys(ws._merges || {});
    for (const key of allMergeKeys) {
        const merge = ws._merges[key];
        if (merge && merge.model && merge.model.top >= 3) {
    ...
    if (ws._merges) {
        for (const key of Object.keys(ws._merges)) {
            ...
            if (m && m.model && m.model.top >= 3) {
                delete ws._merges[key];
            }
        }
    }

    // 2b. Move Logo 2 (the right hexagons) to the new right-most column
    // The template has it around col G/H (7/8). We want it at COL_TOTAL_JAM (approx).
    const images = ws.getImages();
    images.forEach(img => {
        // If anchored around column 6-7 (zero-indexed F-G), move it to COL_TOTAL_JAM - 1 area
        if (img.range.tl.col >= 5) {
            const widthCols = img.range.br.col - img.range.tl.col;
            img.range.tl.col = COL_TOTAL_JAM - widthCols - 1;
            img.range.br.col = COL_TOTAL_JAM - 1;
        }
    });

    // 3. Re-merge header rows 11-14 across new column range (no spliceColumns needed)
    //    ExcelJS auto-extends when we write to columns beyond the template's 8
    ...
    // 5. Re-merge rows 3-14 across full width or center
    const lastCol = COL_TOTAL_JAM;
    
    // Rows 3-9: Institution Address/Email (Center area, offset for logos)
    for (let r = 3; r <= 9; r++) {
        // Find the "real" value in the row (sometimes it's in C1, sometimes C3)
        let rowVal = '';
        for (let c = 1; c <= 8; c++) {
        ...
        // Clear columns 1 and 2 to remove any text from the far left
        ws.getCell(r, 1).value = '';
        ws.getCell(r, 2).value = '';
        
        // Put value into C3 (start of merge)
        const cellCenter = ws.getCell(r, 3);
        cellCenter.value = rowVal;
        
        // Offset col 3 to lastCol-2 to leave space for logos
    ...
    const blueRowStyle = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FF4F81BD' } // Royal blue or match template
    };
    for(let c = 1; c <= lastCol; c++) {
        ws.getCell(10, c).fill = blueRowStyle;
    }

    // Rows 11-14: Title/Semester/SK/Period (Full width)
    for (let r = 11; r <= 14; r++) {
